...
class StageUI
  ...
  # highlight the users attention to a specific polygon by flashing
  # its area
...
class Controller
  constructor: (args) ->
    ...
    update_category = (category) => 
      ...
      if @curr_attribute.length > 0
        $('#btn-submit').removeAttr('disabled')
        

    update_attribute = (attribute) => 
      @curr_attribute.push attribute
    ...
    $('.cat').on('mouseup', -> update_category($(@).find('a').text()))

    $('.att').on('mouseup', -> update_attribute($(@).find('a').text()))
    ...
    $('#btn-submit').on('click', => 
      ...
      btn_submit(@curr_category,@curr_attribute))
